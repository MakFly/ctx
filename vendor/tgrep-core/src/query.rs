...
pub enum QueryPlan {
    ...
    /// Every child plan must match, including nested alternatives.
    All(Vec<QueryPlan>),
        ...
        HirKind::Concat(subs) => {
            ...
            for sub in subs {
                if let HirKind::Literal(Literal(bytes)) = sub.kind() {
                    ...
                    match sub_plan {
                        QueryPlan::And(queries) => all_queries.extend(queries),
                        QueryPlan::MatchAll => {}
                        other => constraints.push(other),
                    }
                    // Keep alternatives as conjuncts; distributing them would
                    // grow exponentially for multiple groups.
                }
            }
            ...
            if !all_queries.is_empty() {
                constraints.push(QueryPlan::And(all_queries));
            }
            simplify(QueryPlan::All(constraints))
        }
...
fn simplify(plan: QueryPlan) -> QueryPlan {
    match plan {
        ...
        QueryPlan::All(plans) => {
            ...
            for plan in plans.into_iter().map(simplify) {
                match plan {
                    QueryPlan::MatchAll => {}
                    QueryPlan::And(queries) => trigrams.extend(queries),
                    other => constraints.push(other),
                }
            }
            if !trigrams.is_empty() {
                constraints.push(simplify(QueryPlan::And(trigrams)));
            }
            match constraints.len() {
                0 => QueryPlan::MatchAll,
                1 => constraints.pop().unwrap(),
                _ => QueryPlan::All(constraints),
            }
        }
        QueryPlan::Or(plans) => {
            let simplified: Vec<QueryPlan> = plans.into_iter().map(simplify).collect();
            if simplified.iter().any(QueryPlan::is_match_all) {
                return QueryPlan::MatchAll;
            }
            if simplified.len() == 1 {
    ...
    F: Fn(TrigramHash) -> Vec<u32>,
{
    match plan {
        ...
        QueryPlan::All(plans) => intersect_many_sorted(
            plans
                .iter()
                .filter(|p| !p.is_match_all())
                .map(|sub| execute_plan(sub, lookup))
                .collect(),
        ),
        QueryPlan::Or(plans) => {
...
fn intersect_many_sorted(mut lists: Vec<Vec<u32>>) -> Vec<u32> {
...
mod tests {
    ...
    #[test]
    fn nested_alternatives_filter_code_without_losing_matches() {
        let documents = [
            "async def solve_dependencies(): pass",
            "def get_dependant(): pass",
            "def unrelated(): pass",
            "@app.get('/')",
            "@router.post('/')",
            "@unrelated.get('/')",
            "prefix alpha suffix gamma",
            "prefix beta suffix delta",
            "prefix alpha suffix unrelated",
            "prefix x suffix gamma",
        ];
        let mut postings = std::collections::HashMap::<_, Vec<PostingEntry>>::new();
        for (id, content) in documents.iter().enumerate() {
            for (hash, masks) in trigram::extract_with_masks(content.as_bytes()) {
                postings.entry(hash).or_default().push(PostingEntry {
                    file_id: id as u32,
        ...
        for (pattern, expected_ids) in [
            (r"def (solve_dependencies|get_dependant)\(", vec![0, 1]),
            (r"@(app|router)\.(get|post)\(", vec![3, 4]),
            (r"prefix (alpha|beta) suffix (gamma|delta)", vec![6, 7]),
            (r"prefix (alpha|x) suffix gamma", vec![6, 8, 9]),
        ] {
            let plan = build_query_plan(pattern, false).unwrap();
            let candidates = execute_plan_with_masks(&plan, &|hash| {
                postings.get(&hash).cloned().unwrap_or_default()
            });
            let plain = execute_plan(&plan, &|hash| {
                postings
                    .get(&hash)
                    .into_iter()
                    .flatten()
            ...
            // The short alternative intentionally weakens pruning, never recall.
            let regex = regex::Regex::new(pattern).unwrap();
            for (id, doc) in documents.iter().enumerate() {
                if regex.is_match(doc) {
                    assert!(candidates.contains(&(id as u32)), "{pattern}: {doc}");
                    assert!(plain.contains(&(id as u32)), "{pattern}: {doc}");
                }
            }
            if !pattern.contains("|x") {
                assert_eq!(candidates, expected_ids, "{pattern}");
                assert_eq!(plain, expected_ids, "{pattern}");
            }
        }
    }
