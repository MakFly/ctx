...
/// ctx adapter: stream the exact decoded documents stored in the SQLite transaction.
/// No source file is reread, and only one document's masks are held at a time.
...
impl DocumentBuilder {
    pub fn new(root: &Path, directory: &Path, buffer_bytes: usize) -> Result<Self> {
        std::fs::create_dir_all(directory)?;
        Ok(Self {
            root: root.to_path_buf(),
            directory: directory.to_path_buf(),
            paths: Vec::new(),
            sorter: ExternalSorter::new(directory, buffer_bytes),
        })
    }

    pub fn push(&mut self, path: String, content: &str) -> Result<()> {
        let id = u32::try_from(self.paths.len())
            .map_err(|_| Error::IndexCorrupted("too many documents".into()))?;
        self.sorter.push_file(
            id,
            crate::live::LiveIndex::compute_trigram_masks(content.as_bytes()),
        )?;
    ...
    pub fn finish(self) -> Result<()> {
        let (trigrams, _) = self.sorter.write_postings(&self.directory)?;
        write_files_and_meta(
            &self.directory,
            &self.root,
            self.paths.len(),
